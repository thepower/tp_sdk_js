...
Use this method to send signed transaction to the node.
...
Method returns node's response or throws exception if network error occurred.
...
await tpSdk.transactionsLib.sendTx(packedTx, 'http://wallet.thepower.io/api/chain/3')
```

will produce (assuming no network error occurs) object similar to

```
{
  result: "ok",
  txid: "2Nxios3hijA7Xow6Tv5-c3n4",
  ok: true
}
```
...
```js
getTxStatus(txId, baseURL)
```

Use this method to retrieve transaction's status from node.
Keep in mind that transaction's status is discarded in two minutes after transaction is processed and can no longer be retrieved.
...
const {txid} = await tpSdk.transactionsLib.sendTx(packedTx, 'http://wallet.thepower.io/api/chain/3')

setTimeout(async () => console.log(await tpSdk.transactionsLib.getTxStatus(txid, 'http://wallet.thepower.io/api/chain/3')), 3000);
```
